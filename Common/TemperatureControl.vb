Public Class TemperatureControl
  ...
  <Translate("zh-TW", "超溫")> Public Alarms_TemperatureHigh As Boolean
  <Translate("zh-TW", "溫度不足")> Public Alarms_TemperatureLow As Boolean
...
#End If
  ...
  ' TODO: This next one is read by other commands, but not by us - this is a bit odd
    <Translate("zh-TW", "TC:升降溫模式(比例閥專用)"), Category("Temperature Control"), _
    TranslateCategory("zh-TW", "溫度控制"), DefaultValue(0), _
    Description("比例閥專用,升溫閥常開設定1")> _
    Public Parameters_HeatCoolModeChange As Integer = 0
  ...
  <Translate("zh-TW", "TC:急速降溫到達溫度"), Category("Temperature Control"), TranslateCategory("zh-TW", "溫度控制"), DefaultValue(800), Unit(Unit.TemperatureTenths)> _
    Public Parameters_CrashCoolTemperature As Integer = 800
    ...
    With ControlCode
      ...
      If Not IgnoreErrors Then
                ...
                If .CoolNow = True Then
                    If (.Setpoint - .IO.MainTemperature) > 100 Then
                        pidReset(.IO.MainTemperature)
                    End If
                End If
  ...
  Private Sub MakeAlarms(ByVal vesTemp As Integer, ByVal ignoreErrors As Boolean)
    ...
    If vesTemp < (TempSetpoint + Parameters_TemperatureAlarmBand) Then
      TempHiAlarmTimer.TimeRemaining = Parameters_TemperatureAlarmDelay
    End If
    'Alarms_TemperatureLow = TempLoAlarmTimer.Finished And ((pid_.GradientTerm > 0) Or pid_.IsHolding)
        'Alarms_TemperatureHigh = TempHiAlarmTimer.Finished And ((pid_.GradientTerm > 0) Or pid_.IsHolding)
...
Public Class PidControl
  ...
  Public Sub Run(ByVal currentTempInTenths As Integer)
    ...
    If (Not stopIntegral) And errorSumTimer_.Finished Then
      ...
      '========================直升的時候，到目標剩下1.5度，將errorSum_ * 0.8
      If IsMaxGradient And IsHolding And (finalTemp_ - currentTempInTenths) < 9 And finalTemp_ > currentTempInTenths And gradient_ = 0 And HoldCut = False Then
        HoldCut = True
        errorSum_ = CType(errorSum_ * 0.3, Integer)
      End If
      '========================
      errorSum_ += tempError
    End If
    ...
    integralTerm_ = Math.Min(Math.Max((errorSum_ * integral_) \ 6000, -1000), 1000)
