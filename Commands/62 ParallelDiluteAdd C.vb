'<Command("ParallelDiluteAdd C", "Time |0-60|", , , "'1", CommandType.ParallelCommand), _
' TranslateCommand("zh-TW", "平行C稀釋加藥", "加藥時間 |0-60|"), _
    ...
    With ControlCode
      ...
      Select Case State
        ...
        Case S62.WaitNoAddButtons
          ...
          If .Command55.IsOn Or .Command65.IsOn Or .Command61.IsOn Then Exit Select
        ...
        Case S62.WaitNotTankLowLevel
          ...
          If (.CTankLowLevel And .Parameters.C藥缸水位種類 = 0) Or (.Parameters.C藥缸水位種類 = 1 And .IO.CTankLow) Then Wait.TimeRemaining = .Parameters.AddTransferTimeBeforeRinse
          If Wait.Finished Then
        ...
        Case S62.WaitNotTankLowLevel2
          StateString = If(.Language = LanguageValue.ZhTw, "C缸加藥中", "Transferring C ") & TimerString(Wait.TimeRemaining)
          If (.CTankLowLevel And .Parameters.C藥缸水位種類 = 0) Or (.Parameters.C藥缸水位種類 = 1 And .IO.CTankLow) Then Wait.TimeRemaining = .Parameters.AddTransferTimeAfterRinse
          If Wait.Finished Then
            Wait.TimeRemaining = .Parameters.AddTransferRinseTime
        ...
        Case S62.WaitAddFinish3
          ...
          If (.CTankLowLevel And .Parameters.C藥缸水位種類 = 0) Or (.Parameters.C藥缸水位種類 = 1 And .IO.CTankLow) Then Wait.TimeRemaining = .Parameters.AddTransferDrainTime
          If Wait.Finished Then
...
#Region "Standard Definitions"
  ...
  Public ReadOnly Property IsTransfer() As Boolean
    Get
      Return ((State = S62.WaitAddTimeHold) Or (State = S62.WaitAddTimeTranfer) Or (State = S62.WaitNotTankLowLevel) Or (State = S62.WaitWashSTank) Or _
              (State = S62.WaitNotTankLowLevel2))
    End Get
  End Property
  Public ReadOnly Property IsMixing() As Boolean
    Get
      Return ((State = S62.WaitTankReady) Or (State = S62.WaitAddTimeRunback) Or (State = S62.WaitAddTimeHold) Or (State = S62.WaitAddTimeTranfer))
    End Get
  End Property
  Public ReadOnly Property IsRinsing() As Boolean
    Get
      Return ((State = S62.WaitWashSTank) Or (State = S62.WaitWashSTank2))
    End Get
  End Property
